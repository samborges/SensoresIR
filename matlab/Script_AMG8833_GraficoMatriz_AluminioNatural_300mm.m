...
% Nome do arquivo CSV
filename = 'arrays\temperatura_AMG_8833\Array_DadosAluminioNatural_300mm_AMG8833.txt';
...
title('Sensor AMG 8833 (Aluminio Natural 300mm)- Mapa de Calor (°C)');
colormap('cool')
xlabel('Base');
ylabel('Laterais');
...
temperaturaCamera = 30.5;
temperaturaTermometro = 44;

% Adicionar o valor médio no gráfico
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = temperaturaMediaMovel - 1 ; % Ajustar a posição Y acima da linha
...
ylim([25 50]);
title('Comparação dos Dados Originais e Filtrados (Aluminio Natural 300mm)');
xlabel('Tempo seg');
ylabel('Valor °C');

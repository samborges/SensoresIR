...
% Nome do arquivo CSV
filename = 'arrays\temperatura_AMG_8833\Array_DadosPlasticoLiquidoBranco_300mm_AMG8833.txt';
...
title('Sensor AMG 8833 (Plástico Branco 300mm)- Mapa de Calor (°C)');
colormap('cool')
xlabel('Base');
ylabel('Laterais');
...
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = temperaturaCamera + 3 ; % Ajustar a posição Y acima da linha
...
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = temperaturaTermometro - 3; % Ajustar a posição Y acima da linha
...
ylim([30 60]);
title('Comparação dos Dados Originais e Filtrados (Plástico Branco 300mm)');
xlabel('Tempo seg');
ylabel('Valor °C');

...
% Nome do arquivo CSV
filename = 'arrays\temperatura_AMG_8833\Array_DadosVidroLiquidoBranco_300mm_AMG8833.txt';
...
title('Sensor AMG 8833 (Vidro Branco 300mm)- Mapa de Calor (°C)');
colormap('cool')
xlabel('Base');
ylabel('Laterais');
...
temperaturaCamera = 50.4;
temperaturaTermometro = 53;
...
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = temperaturaCamera + 1 ; % Ajustar a posição Y acima da linha
...
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = temperaturaTermometro +1; % Ajustar a posição Y acima da linha
...
ylim([30 60]);
% title('Comparação dos Dados Originais e Filtrados (Vidro Branco 300mm)');
xlabel('Tempo seg');
ylabel('Valor °C');
...
variacao_percentual_1 = ((temperaturaCamera - temperaturaMediaMovel) / temperaturaMediaMovel) * 100;
variacao_percentual_2 = ((temperaturaTermometro - temperaturaMediaMovel) / temperaturaMediaMovel) * 100;
% Exibição do resultado
fprintf('A variação percentual entre %.2f e %.2f é %.2f%%\n', temperaturaMediaMovel, temperaturaCamera, variacao_percentual_1);

fprintf('A variação percentual termometro entre %.2f e %.2f é %.2f%%\n', temperaturaMediaMovel, temperaturaTermometro, variacao_percentual_2);

...
% Nome do arquivo CSV
filename = 'arrays\distancia_VL53L5cx\Array_dadosIniciais_VL53L5cx.txt';
...
title('Sensor VL53L5 (Vazio) - Mapa distância (mm)');
colormap(flip(cool));
...
yline(distanciaMediaMovel, '-', 'Color', [1, 0.00, 0.00], 'LineWidth', 1.5, 'DisplayName', 'Valor Médio', 'Alpha',0.5);

distancia = 200;

% Adicionar o valor médio no gráfico
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = distanciaMediaMovel + 20 ; % Ajustar a posição Y acima da linha
text(x_position, y_position, sprintf('Distância Média Móvel: %.2f', distanciaMediaMovel),'HorizontalAlignment', 'center', 'FontSize', 10, 'Color', [0.0, 0.0, 0.0]);
% Adicionar a linha de referência de distancia
yline(distancia, ':', 'Color', [0.0, 1, 0.0], 'LineWidth', 1, 'DisplayName', 'Câmera Térmica');
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = distancia - 20 ; % Ajustar a posição Y acima da linha
text(x_position, y_position, sprintf('Distância na régua do Sensor: %.2f', distancia),'HorizontalAlignment', 'center', 'FontSize', 10, 'Color', [0.0, 0.0, 0.0]);

ylim([150 500]);
title('Sensor VL53L5 - Comparação dos Dados Originais e Filtrados (Vazio)');

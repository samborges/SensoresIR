...
title('Sensor VL53L5 (Plastico Transparente 300mm) - Mapa distância (mm)');
colormap(flip(cool));
...
y_position = distanciaMediaMovel - 25 ; % Ajustar a posição Y acima da linha
...
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = distancia + 10 ; % Ajustar a posição Y acima da linha
...
ylim([280 580]);
title('Sensor VL53L5 - Comparação dos Dados Originais e Filtrados (Plastico Transparente 300mm)');

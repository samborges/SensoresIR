...
% Nome do arquivo CSV
filename = 'arrays\distancia_VL53L5cx\Array_VidroLiquidoTransparente_300mm_VL53L5.txt';
...
title('Sensor VL53L5 (Vidro Transparente 300mm) - Mapa distância (mm)');
colormap(flip(cool));
...
y_position = distanciaMediaMovel - 15 ; % Ajustar a posição Y acima da linha
text(x_position, y_position, sprintf('Distância Média Móvel: %.2f', distanciaMediaMovel),'HorizontalAlignment', 'center', 'FontSize', 12, 'Color', [0.0, 0.0, 0.0]);
% Adicionar a linha de referência de distancia
yline(distancia, ':', 'Color', [1, 0, 0], 'LineWidth', 2, 'DisplayName', 'Distância Régua');
x_position = length(colunaRelevante)/2; % Posição X para o texto
y_position = distancia - 7; % Ajustar a posição Y acima da linha
text(x_position, y_position, sprintf('Distância na Régua do Sensor: %.2f', distancia),'HorizontalAlignment', 'center', 'FontSize', 12, 'Color', [0.0, 0.0, 0.0]);

ylim([280 580]);
% title('Sensor VL53L5 - Comparação dos Dados Originais e Filtrados (Vidro Transparente 300mm)');
...
variacao_percentual_1 = (( distanciaMediaMovel*100) / distancia)  - 100;
% Exibição do resultado
fprintf('A variação percentual entre %.2f e %.2f é %.2f%%\n', distanciaMediaMovel, distancia, variacao_percentual_1);
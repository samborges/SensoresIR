...
% Nome do arquivo CSV
filename = 'arrays\Array_DadosVidroBranco_30cm_VL53L5.txt';
...
filtro_media_movel = movmean(dadoOriginal, 10);

% Calcular a média dos dados da quarta coluna
media_dadoOriginal = mean(dadoOriginal);

% Criar o gráfico de linha comparando os dados originais e filtrados
figure;
hold on;
plot(dadoOriginal, 'b-.', 'Color', [0.0, 0.0, 1], 'LineWidth', 0.5,'DisplayName', 'Dados Originais');
plot(filtro_media_movel, '-', 'Color', [1, 0.0, 0.0], 'LineWidth', 2, 'DisplayName', 'Média Móvel');
yline(media_dadoOriginal, '-', 'Color', [0.93, 0.69, 0.13], 'LineWidth', 2, 'DisplayName', 'Valor Médio');
ylim([150 350]);
